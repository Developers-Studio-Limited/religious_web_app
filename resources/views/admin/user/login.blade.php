...
<script>
  ...
  gtag('config', 'UA-37564768-1');
	...
	<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" style="background-image: url(admin_assets/img/bg/bg-1.jpg);">
				...
				<div class="kt-login__signin">
					...
					<form class="kt-form" method="POST" action="{{ route('login') }}">
						...
						<div class="input-group">
							<input class="form-control @error('email') is-invalid @enderror" type="text" placeholder="Email"  name="email" value="{{ old('email') }}" required autofocus autocomplete="off">
							@error('email')
								<span class="invalid-feedback" role="alert">
									<strong>{{ $message }}</strong>
									@if(session('errorAdmin'))
										<strong>{{session('errorAdmin')}}</strong>
									@endif
								</span>
							@enderror
							...
							@if(session('error_admin'))
								<span class="invalid-feedback" role="alert" style="display:revert ">
										<strong>{{session('error_admin')}}</strong>
								</span>
							@endif
						</div>
						<div class="input-group">
